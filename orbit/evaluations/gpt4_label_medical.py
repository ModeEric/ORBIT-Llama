...
def construct_prompt(text):
    """
    Constructs the prompt with the given text.
    """
    prompt = f"""Please evaluate the educational value of the following medical-related text from a web document. Use this 6-point scoring system:
0 points: No medical content at all.
1 point: Minimal medical information, or medical information mixed with non-medical content.
2 points: Covers basic medical concepts but lacks depth or comprehensive explanation.
3 points: Clear explanation of concepts with relevant examples, educational for a general audience.
4 points: In-depth knowledge, covers advanced medical principles or recent medical developments, well-structured and engaging.
5 points: Exceptionally high educational value, expert-level insights, connects multiple medical concepts, addresses misconceptions, inspires further learning.
    ...
    for attempt in range(1, max_retries + 1):
        try:
            response = openai.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": "You are an assistant that evaluates educational content."},
...
def main(file_path, output_file="evaluation_results.jsonl", model="gpt-4o", max_entries=1000):
    """
    Main function to evaluate the educational value of a single JSONL file.
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python evaluate_single_file.py <path_to_file>")
        sys.exit(1)
